...
def on_connect(client, userdata, flags, rc):
    device = myapi.get_device_details()
    for entry in device:
        devid = entry.id
...
for entry in device:
    ...
    switch = "sengled/"+devid+"/switch"
    status = "sengled/"+devid+"/status"
    homeassistant.insert(i,{"platform":"mqtt","name":name,"state_topic":status,"command_topic":switch,"payload_on":"ON","payload_off":"OFF","optimistic":False})
    i = i+1
